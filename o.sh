...
function safe_base64() {
  ...
  # this gives the following errors on Mac:
...
function check_firewall() {
  # This is incorrect if access keys are using more than one port.
...
install_shadowbox() {
  ...
  # if the script fails after docker run, it will continue to fail
  ...
  # Don't wait for Shadowbox to run this.
